<template>
  <div id="preview">
    <trashed-message v-if="unit.deleted_at" class="mb-5" :can-restore="authorize.restore" @restore="restore"> This unit has been deleted. </trashed-message>
    <div class="w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 p-5 mb-5">
      <h1 class="font-semibold text-2xl">Units</h1>
      <div class="flex">
        <!-- breadcrumb -->
    ...
    <div class="bg-white overflow-x-auto">
      <table class="w-full text-left">
        <tbody class="text-gray-600 text-sm font-light">
          ...
          <tr class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Parent System</td>
            <td class="py-3 px-6">
              <span>{{ unit.system.name }}</span>
            </td>
          </tr>
          <tr class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Type of Unit</td>
            <td class="py-3 px-6">
              <span>{{ unit.type }}</span>
            </td>
          </tr>
          <tr class="border-b border-gray-200 hover:bg-gray-100" v-if="unit.conversion_reference">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Conversion Factor<small> (With respect to BASE STANDARD)</small></td>
            <td class="py-3 px-6">
              {{ unit.conversion_factor }} <span v-if="unit.conversion_reference">{{ unit.conversion_reference.name }} ({{ unit.conversion_reference.short_hand }})</span>
            </td>
          </tr>
          <tr class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Created At</td>
            <td class="py-3 px-6">
              <span>{{ unit.created_at }}</span>
            </td>
          </tr>
        </tbody>
      </table>
