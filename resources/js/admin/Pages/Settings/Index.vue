    ...
    <div class="bg-white overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="p-5 -mr-5 -mb-2 flex flex-wrap">
          <text-input v-model="form.app_name" :error="form.errors.app_name" :required="true" class="pr-5 pb-2 w-full lg:w-1/2" label="App Name" />
          <text-input v-model="form.business_email" type="email" :error="form.errors.business_email" class="pr-5 pb-2 w-full lg:w-1/2" label="Business Email" />
        ...
        <div class="p-5 -mr-5 -mb-2 flex flex-wrap">
          <text-input v-model="form.from_email" type="email" :error="form.errors.from_email" :required="true" class="pr-5 pb-2 w-full lg:w-1/2" label="From Email" />
          <text-input v-model="form.email_from_name" :error="form.errors.email_from_name" :required="true" class="pr-5 pb-2 w-full lg:w-1/2" label="Email From Name" />
        </div>
        ...
        <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center">
          <loading-button v-if="authorize.update" :loading="form.processing" class="btn-indigo ml-auto" type="submit">Save</loading-button>
        </div>
      </form>
    </div>
  </div>
