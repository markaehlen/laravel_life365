<template>
  <div id="preview">
    <trashed-message v-if="exposure.deleted_at" class="mb-5" :can-restore="authorize.restore" @restore="restore">
      This exposure has been deleted.
    </trashed-message>
    <div class="w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 p-5 mb-5">
      <h1 class="font-semibold text-2xl">Exposures</h1>
      <div class="flex">
        <!-- breadcrumb -->
        <nav class="text-sm font-semibold" aria-label="Breadcrumb">
          <ol class="list-none p-0 inline-flex">
            ...
            <li class="flex items-center text-indigo-500">
              <inertia-link :href="route('admin.locations.index')" class="text-gray-700">
                Exposures
              </inertia-link>
              <icon name="cheveron-right" class="fill-current w-4 h-4 mx-2 mt-1" />
            </li>
    ...
    <div class="px-5 py-5 bg-gray-50 border-t border-gray-100 flex  items-center">
      <action-back :href="route('admin.exposures.index')" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    restore() {
      ...
      swal('Are you sure you want to restore this exposure?', {
        buttons: ['No', 'Yes'],
      }).then(value => {
        if (value) {
          vInstance.$inertia.put(vInstance.route('admin.exposures.restore', vInstance.exposure.id))
        }
      })
    },
  },
