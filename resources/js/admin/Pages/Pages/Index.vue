<template>
  <div id="pages">
    <div class="w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 p-5 mb-5">
      <h1 class="font-semibold text-2xl">CMS Pages</h1>
      <div class="flex">
        <!-- breadcrumb -->
        <nav class="text-sm font-semibold" aria-label="Breadcrumb">
          <ol class="list-none p-0 inline-flex">
            ...
            <li class="flex items-center">
              <a href="javascript:void" class="text-gray-600">CMS Pages</a>
            </li>
          </ol>
        </nav>
        <!-- breadcrumb end -->
    ...
    <div class="mb-5 block 2xl:flex xl:flex lg:flex md:flex justify-between bg-gray-50 py-5 px-5 items-center border-b">
      ...
      <action-add class="add-btn-mobile" :href="route('admin.pages.create')" title="Page" />
    </div>
          ...
          <tr v-for="page in pages.data" :key="page.id" class="border-b border-gray-200 hover:bg-gray-100">
            ...
            <td class="py-3 px-6">
              <div class="flex item-center justify-center">
                <action-preview v-if="authorize.view" :href="route('admin.pages.show', page.id)" title="View" />
                <action-edit v-if="authorize.update" :href="route('admin.pages.edit', page.id)" title="Edit" />
                <action-delete title="Delete" @delete="destroy(page)" />
              </div>
            </td>
          </tr>
          <tr v-if="pages.data.length === 0">
            <td class="border-t px-6 py-4" colspan="4"><b>No pages found.</b></td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    destroy(item) {
      ...
      swal('Are you sure you want to delete this page?', {
        buttons: ['No', 'Yes'],
      }).then(value => {
        if (value) {
          vInstance.$inertia.delete(vInstance.route('admin.pages.destroy', item.id))
        }
      })
    },
  },
