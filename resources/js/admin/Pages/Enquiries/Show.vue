<template>
  <div id="preview">
    <trashed-message v-if="enquiry.deleted_at" class="mb-5" :can-restore="authorize.restore" @restore="restore">
      This website request has been deleted.
    </trashed-message>
    <div class="w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 p-5 mb-5">
      <h1 class="font-semibold text-2xl">View Website Request</h1>
      <div class="flex">
        <!-- breadcrumb -->
      ...
      <table class="w-full text-left">
        <tbody class="text-gray-600 text-sm font-light">
          ...
          <tr v-if="enquiry.phone" class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Phone</td>
            <td class="py-3 px-6">
              <span>{{ enquiry.phone }}</span>
            </td>
          </tr>
          <tr class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Subject</td>
            <td class="py-3 px-6">
              <span>{{ enquiry.subject }}</span>
            </td>
          </tr>
          <tr class="border-b border-gray-200 hover:bg-gray-100">
            <td class="font-semibold text-sm leading-normal py-3 px-6">Message</td>
            <td class="py-3 px-6">
    ...
    <div class="px-5 py-5 bg-gray-50 border-t border-gray-100 flex  items-center">
      <action-back :href="route('admin.enquiries.index')" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    restore() {
      ...
      swal('Are you sure you want to restore this website request?', {
        buttons: ['No', 'Yes'],
      }).then((value) => {
        if(value){
          vInstance.$inertia.put(vInstance.route('admin.enquiries.restore', vInstance.enquiry.id))
        }
      })
    },
  },
