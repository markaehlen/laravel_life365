<template>
  <div id="faqs">
    ...
    <div class="mb-5 flex justify-between bg-gray-50 py-5 px-5 items-center border-b">
      ...
      <action-add v-if="authorize.create" :href="route('admin.faqs.create')" title="Faq" />
    </div>
      ...
      <table class="w-full whitespace-nowrap text-left">
        ...
        <tbody class="text-gray-600 text-sm font-light">
          <tr v-for="faq in faqs.data" :key="faq.id" class="border-b border-gray-200 hover:bg-gray-100">
            <td class="py-3 px-6">
              <span class="text-sm font-medium text-gray-900">{{ faq.srno }}</span>
            </td>
            <td class="py-3 px-6 whitespace-nowrap">
              <span class="text-sm font-medium text-gray-900">{{ faq.title }}</span>
            </td>
            <td class="py-3 px-6">
              <toggle-status :status="faq.is_active" @toggleStatus="toggleStatus(faq)" />
            </td>
            <td class="py-3 px-6">
            ...
            <td class="py-3 px-6">
              <div class="flex item-center justify-center">
                <action-preview v-if="authorize.view" :href="route('admin.faqs.show', faq.id)" title="View" />
                <action-edit v-if="authorize.update" :href="route('admin.faqs.edit', faq.id)" title="Edit" />
                <action-delete v-if="!faq.deleted_at && authorize.delete" title="Delete" @delete="destroy(faq)" />
              </div>
            </td>
          </tr>
  ...
  methods: {
    ...
    destroy(item) {
      ...
      swal('Are you sure you want to delete this faq?', {
        ...
        if (value) {
          vInstance.$inertia.delete(vInstance.route('admin.faqs.destroy', item.id))
        }
      })
    },
    toggleStatus(item) {
      ...
      swal(`Are you sure you want to ${item.is_active ? 'de-activate' : 'activate'} this faq?`, {
        ...
        if (value) {
          vInstance.$inertia.put(vInstance.route('admin.faqs.toggle.status', item.id))
        }
      })
    },
  },
