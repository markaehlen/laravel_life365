<template>
  <div id="help-tips">
    <div class="w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 p-5 mb-5">
      <h1 class="font-semibold text-2xl">Add Tips</h1>
      <div class="flex">
        <!-- breadcrumb -->
    ...
    <div class="bg-white overflow-hidden">
      <form @submit.prevent="page_save">
        <div class="p-5 -mr-5 -mb-2 flex flex-wrap">
          <text-input v-model="form.title" :error="form.errors.title" :required="true" class="pr-5 pb-2 w-full" label="Title/Label" />
          <text-input v-model="form.category" :error="form.errors.category" :required="false" class="pr-5 pb-2 w-full" label="Category" />
          <textarea-input v-model="form.content" :error="form.errors.content" class="pr-5 pb-2 w-full" label="Content" />
        </div>
...
export default {
  ...
  methods: {
    page_save() {
      this.form.post(this.route('admin.help-tips.store'), {
