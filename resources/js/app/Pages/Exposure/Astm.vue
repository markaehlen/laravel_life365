      ...
      <ValidationObserver ref="form">
        <form @submit.prevent="store">
          <div class="w-full border-b flex flex-nowrap items-center bg-gray-50 py-1 px-2.5 mb-5">
            ...
            <span class="btn-indigo-flat ml-auto mobile-back-btn" style="cursor: pointer" @click="$emit('close')">Use Set: {{ form.currentSet }}</span>
          </div>
          <div class="topnav" id="myTopnav" v-if="$page.props.isProjectScreen">
            <a class="text-sm" :href="'javascript:void(0)'" @click="toggleTab()" :class="astmTab == 'sets' ? 'active' : ''">Sets</a>
            <a class="text-sm" :href="'javascript:void(0)'" @click="toggleTab()" :class="astmTab == 'guidance' ? 'active' : ''">Some Guidance</a>
          ...
          <div v-if="astmTab == 'sets'" class="setTab" :key="uuid4">
            <div class="p-5 text-sm flex flex-wrap lg:w-2/5 mod-responsive">
                ...
                <div class="overflow-x-auto w-full">
                  <table class="w-full whitespace-nowrap text-left">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                      <tr v-for="sample in form.sets" :key="sample.name" @click="setCurrentSet(sample.name)" class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-1 px-3">
                        ...
                        <td class="py-1 px-3 whitespace-nowrap text-center">
                          <span class="text-sm font-medium text-gray-900">{{ parseFloat(sample.maxSurfaceConcentration).toFixed(3) }}</span>
                        </td>
              ...
              <div class="subnav" v-if="$page.props.isProjectScreen">
                <a class="text-sm" :href="'javascript:void(0)'" @click="toggleLeftTab()" :class="astmLeftTab == 'setData' ? 'active' : ''">Set Data</a>
                <a class="text-sm" :href="'javascript:void(0)'" @click="toggleLeftTab()" :class="astmLeftTab == 'parameters' ? 'active' : ''">Parameters</a>
                <a href="javascript:void(0);" class="icon" @click="toggleNavbar($event)">
                  <icon name="cheveron-down" class="fill-current w-4 h-4 mx-2 mt-1" />
                </a>
              </div>
              ...
              <div class="text-sm flex w-full flex-wrap" v-if="astmLeftTab == 'setData'">
                <div class="overflow-x-auto w-full">
                  <table class="w-full whitespace-nowrap text-left" :key="uuid3">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                      <tr v-for="(setData, index) in form.selectedSetData" :key="index" class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-1 px-3">
                          <span class="text-sm font-medium text-gray-900">{{ setData.sample }}</span>
                        </td>
                        <td class="py-1 px-3 whitespace-nowrap text-right">
                          <span class="text-sm font-medium text-gray-900">
                            <inline-text-editor :min="0" :max="1000" title="Allowable range of values is [0.000, 1000.000]" :currency-symbol="''" :currency-decimal-places="3" @update="renderASTMGraph(setData.depth, 'depth', index)" :required="true" :close-on-blur="true" :type="'currency'" :value.sync="setData.depth"></inline-text-editor>
                          </span>
                        </td>
                        <td class="py-1 px-3 whitespace-nowrap text-right">
                          <span class="text-sm font-medium text-gray-900">
                            <inline-text-editor :min="0" :max="1.0" title="Allowable range of values is [0.000, 1.000]" :currency-symbol="''" :currency-decimal-places="3" @update="renderASTMGraph(setData.conc, 'conc', index)" :required="true" :close-on-blur="true" :type="'currency'" :value.sync="setData.conc"></inline-text-editor>
                          </span>
                        </td>
                        <td class="py-1 px-3">
                          <span v-if="form.selectedSetData.length > 6" @click="deleteSampleEntry(index)"  style="cursor: pointer"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="text-indigo-400" style="width: 30px"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              <div class="text-sm flex w-full flex-wrap" v-if="astmLeftTab == 'parameters'">
                <number-input :min="0" :max="60000" :rules="`required|min_value:0|max_value:60000`" :title="`Allowable range of values is [0 , 60000]`" v-model="form.days" step="1" :error="form.errors.days" :required="true" class="mt-5 pr-5 w-full lg:w-1/2" :label="'Days'" />
                <number-input :min="0" :max="1" :rules="`required|min_value:0|max_value:1`" :title="`Allowable range of values is [0 , 1]`" v-model="form.ambientConcentration" step="any" :error="form.errors.ambientConcentration" :required="true" class="mt-5 pr-5 w-full lg:w-1/2" :label="'Init. Conc.'" />
                ...
                <textarea-input v-model="form.description" :error="form.errors.description" :required="true" class="pr-5 pb-2 w-full" :label="'Set Description'"></textarea-input>
              </div>
            </div>
            <div class="p-5 text-sm flex flex-wrap lg:w-3/5">
              <div class="subnav" v-if="$page.props.isProjectScreen">
                <a class="text-sm" :href="'javascript:void(0)'" @click="toggleRightTab()" :class="astmRightTab == 'graphs' ? 'active' : ''">Graphs</a>
                <a class="text-sm" :href="'javascript:void(0)'" @click="toggleRightTab()" :class="astmRightTab == 'calculations' ? 'active' : ''">ASTM Calculations</a>
                <a href="javascript:void(0);" class="icon" @click="toggleNavbar($event)">
                  <icon name="cheveron-down" class="fill-current w-4 h-4 mx-2 mt-1" />
                </a>
              </div>
              <div class="text-sm flex w-full flex-wrap" v-if="astmRightTab == 'graphs'">
                <div id="graphCanvas" class="-mr-5 -mb-2 flex flex-wrap w-full">
                  <Chart class="chart2" :key="uuid3" type="scatter" :xLabel="'Depth (mm)'" :yLabel="'Concentration (% wt.conc)'" :title="form.currentSet" :labels="createGraphX()" :datasets="createGraphY()" />
                </div>
                <span class="mt-10 pr-5 pb-2 w-full text-center">
                  <loading-button :loading="form.processing" class="btn-indigo-flat ml-auto mobile-back-btn" type="submit">Calculate ASTM Parameters</loading-button>
                ...
                <div class="mb-10 text-sm flex w-full flex-wrap">
                  <div class="overflow-x-auto w-full">
                    <table class="w-full whitespace-nowrap text-left">
                      ...
                      <tbody class="text-gray-600 text-sm font-light">
                        <tr v-for="(data, index) in form.astmCalcDataTableOneData" :key="index" class="border-b border-gray-200 hover:bg-gray-100">
                          <td class="py-1 px-3">
                  ...
                  <div class="overflow-x-auto w-full">
                    <table class="w-full whitespace-nowrap text-left">
                      ...
                      <tbody v-if="form.astmCalcDataTableTwoData" class="text-gray-600 text-sm font-light">
                        <tr v-for="(data, index) in form.astmCalcDataTableTwoData" :key="index" class="border-b border-gray-200 hover:bg-gray-100">
                          <td class="py-1 px-3">
          ...
          <div v-if="astmTab == 'guidance'">
            <div class="p-5 text-sm flex w-full flex-wrap">
              <strong
                ...
                Standard Test Method for Determining the Apparent Chloride Diffusion Coefficient of Cementitious Mixtures by Bulk Diffusion</strong
              >
            </div>
            <div class="p-5 text-sm flex w-full flex-wrap">Life-365 allows users to input chloride concentration data taken from field samples. Life-365 does not require that the data be gathered this way, but rather that it simply conforms to the number and depth of samples that can be created using the ASTM process. The ASTM Standard Method suggests that data be gathered at the following intervals, where thie table expresses values in millimeters (mm). (1 inch = 25.4mm)</div>
            <div class="p-5 text-sm flex w-full flex-wrap justify-center"><strong class="text-center">Table of Suggested Measurement Depths, by w/cm (source: ASTM)</strong></div>
            <div class="p-5 text-sm flex w-full flex-wrap justify-center"><ag-grid-vue @grid-ready="onGridReady" :defaultColDef="defaultColDef" :autoSizeAllColumns="true" :headerHeight="30" :rowHeight="40" style="width: 70%; height: 90%" :domLayout="`autoHeight`" class="ag-theme-alpine" :columnDefs="guidanceTableHeaders" :rowData="guidanceTableData"> </ag-grid-vue></div>
            <div class="p-5 text-sm flex w-full flex-wrap">Life-365 does follow the ASTM C1556 procedure for estimating the maximum surface chloride concentration from this data. It then allows the user to use this estimate as the maximum chloride concentration used in the Exposure tab. The user, however, still needs to input their own estimate of the time in years for this concentration to be reached.</div>
            <div class="p-5 text-sm flex w-full flex-wrap">The procedure also estimates a bulk diffusion coefficient for the concrete. This estimate is based on Fick's Second Law-based model, NOT Life-365, and thus is not used by Life-365 in its service life calculations.</div>
            <div class="p-5 text-sm flex w-full flex-wrap">For more information about the ASTM C1556 technique, see the ASTM Publication of this standard test method, as well as the Life-365 Users Manual.</div>
          </div>
        </form>
      </ValidationObserver>
    </div>
