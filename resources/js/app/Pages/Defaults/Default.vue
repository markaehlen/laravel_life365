<template>
  <div id="default" class="document">
    ...
    <div class="flex w-full border-b block 2xl:flex xl:flex lg:flex md:flex flex-nowrap justify-between items-center bg-gray-50 py-1 px-2.5">
      <h1 class="font-semibold text-2xl">Defaults Settings & Parameters</h1>
      <div class="flex">
              <loading-button id="save-button" :loading="form.processing" type="submit" class="btn-indigo-flat mr-2">OK</loading-button>
            </div>
    </div>
    <div class="bg-white overflow-hidden1">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 md:grid-cols-2 gap-4 p-2.5">
            <select-input v-model="form.baseUnits" :rules="`required`" :title="`Choose base system units`" :tool-tip-text="helping_tips.base_units" :error="form.errors.baseUnits" class="selectTyp" :required="true" :label="'Base Units'" @input="getRedefinedUnits($event, ecoParameters)">
              <option v-for="(system, index) in systemsOptions" :key="index" :value="index">{{ system }}</option>
            </select-input>
            <select-input v-model="form.concUnits" :rules="`required`" :title="`Select concentration units`" :error="form.errors.concUnits" class="selectTyp" :required="true" :label="'Concentration Units'" :tool-tip-text="helping_tips.concentration_units">
              <option v-for="(concentration_unit, index) in concentrationUnitsOptions" :key="index" :value="index">{{ concentration_unit }}</option>
            </select-input>
          </div>
          <div class="w-full border-b flex flex-nowrap items-center bg-gray-50 py-1 px-2.5">
            <h3 class="font-semibold">Default Region and Exposure Conditions</h3>
          </div>
          <div class="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 gap-4 p-2.5">
            <select-input id="location" v-model="form.location" :rules="`required`" :title="`Select location`" :error="form.errors.location" class="selectTyp" :required="true" :label="'Location'" :tool-tip="helping_tips.location" @input="getSublocations($event)">
              <option v-for="(location, index) in locationOptions" :key="index" :value="index">{{ location }}</option>
            </select-input>
            <select-input id="sub-location" v-model="form.subLocation" :rules="`required`" :title="`Select Sub-location`" :error="form.errors.subLocation" class="selectTyp" :required="true" :label="'Sub-locations'" :tool-tip-text="helping_tips.sub_locations" @input="getExposures($event)">
              <option v-for="(sublocation, index) in sublocationOptions" :key="index" :value="index">{{ sublocation }}</option>
            </select-input>
            <select-input id="exposure" v-model="form.exposureType" :rules="`required`" :title="`Select exposure type`" :error="form.errors.exposureType" class="selectTyp" :required="true" :label="'Exposure'" :tool-tip-text="helping_tips.exposure_default_settings">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-4 md:grid-cols-4 gap-4 p-2.5">
            <number-input v-model="form.baseYear" :tool-tip-text="helping_tips.base_year_default_settings" :min="2000" :max="`${new Date().getFullYear() + 2}`" :rules="`required|min_value:2000|max_value:${new Date().getFullYear() + 2}`" :title="`Allowable range of values is [2000 , ${new Date().getFullYear() + 2}]`" step="1" :error="form.errors.baseYear" :required="true" class="selectTyp" label="Base Year (e.g. 2023)" @blur="validateAndUpdateEcoParams('baseYear', 'Base Year', $event, 2000, new Date().getFullYear() + 2)" />
            <number-input v-model="form.studyPeriod" :tool-tip-text="helping_tips.analysis_period_default_settings" :min="1" :max="500" :rules="`required|min_value:1|max_value:500`" :title="`Allowable range of values is [1 , 500]`" step="1" :error="form.errors.studyPeriod" :required="true" class="selectTyp mb-2.5" label="Analysis Period (yrs)" @blur="validateAndUpdateEcoParams('studyPeriod', 'Study Period', $event, 1, 500)" />
            <number-input v-model="form.inflation" :tool-tip-text="helping_tips.inflation_rate_default_settings" :min="0" :max="20" :rules="`required|min_value:0|max_value:20`" :title="`Allowable range of values is [0% , 20%]`" step="any" :error="form.errors.inflation" :required="true" class="selectTyp mb-2.5" label="Inflation Rate (%)" @blur="validateAndUpdateEcoParams('inflation', 'Inflation', $event, 0, 20)" />
            <number-input v-model="form.discount" :tool-tip-text="helping_tips.discount_rate_default_settings" :min="0" :max="20" :rules="`required|min_value:0|max_value:20`" :title="`Allowable range of values is [0% , 20%]`" step="any" :error="form.errors.discount" :required="true" class="selectTyp mb-2.5" label="Discount Rate (%)" @blur="validateAndUpdateEcoParams('discount', 'Discount Rate', $event, 0, 20)" />
          </div>
          <div class="grey-bg-block grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-4 p-2.5">
            <div class="colGrey">
              ...
              <number-input v-model="form.baseMixCost" :tool-tip-text="helping_tips.concrete_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.baseMixCost" :required="true" class="selectTyp mb-2.5" :label="'Concrete ($/' + volume_unit + ')'" @blur="validateAndUpdateEcoParams('baseMixCost', 'Base Mix Cost', $event, 0, 10000)" />
              <number-input v-model="form.costBlackSteel" :tool-tip-text="helping_tips.black_steel_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costBlackSteel" :required="true" class="selectTyp mb-2.5" :label="'Black Steel ($/' + weight_unit + ')'" @blur="validateAndUpdateEcoParams('costBlackSteel', 'Black Steel Cost', $event, 0, 10000)" />
              <number-input v-model="form.costEpoxy" :tool-tip-text="helping_tips.epoxy_coated_steel_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costEpoxy" :required="true" class="selectTyp mb-2.5" :label="'Epoxy Coated Steel ($/' + weight_unit + ')'" @blur="validateAndUpdateEcoParams('costEpoxy', 'Epoxy Steel Cost', $event, 0, 10000)" />
              <number-input v-model="form.costStainless" :tool-tip-text="helping_tips.stainless_steel_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costStainless" :required="true" class="selectTyp mb-2.5" :label="'Stainless Steel ($/' + weight_unit + ')'" @blur="validateAndUpdateEcoParams('costStainless', 'Stainless Steel Cost', $event, 0, 10000)" />
            </div>
            <div class="colGrey">
              <h5 class="pr-5 w-full font-semibold"><u>Barriers & Inhibitors</u></h5>
              <number-input v-model="form.costMembrane" :tool-tip-text="helping_tips.membrane_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costMembrane" :required="true" class="selectTyp mb-2.5" :label="'Membrane ($/' + area_unit + ')'" @blur="validateAndUpdateEcoParams('costMembrane', 'Membrane Cost', $event, 0, 10000)" />
              <number-input v-model="form.costSealer" :tool-tip-text="helping_tips.sealer_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costSealer" :required="true" class="selectTyp mb-2.5" :label="'Sealer ($/' + area_unit + ')'" @blur="validateAndUpdateEcoParams('costSealer', 'Sealer Cost', $event, 0, 10000)" />
              <number-input v-model="form.costInhibitor" :tool-tip-text="helping_tips.inhibitor_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.costInhibitor" :required="true" class="selectTyp mb-2.5" :label="'Inhibitor ($/' + capacity_unit + ')'" @blur="validateAndUpdateEcoParams('costInhibitor', 'Inhibitor Cost', $event, 0, 10000)" />
            </div>
            <div class="colGrey">
              <h5 class="selectTyp font-semibold"><u>Repairs</u></h5>
              <number-input v-model="form.repairCost" :tool-tip-text="helping_tips.repair_default_settings" :min="0" :max="10000" :rules="`required|min_value:0|max_value:10000`" :title="`Allowable range of values is [0 , 10000]`" step="any" :error="form.errors.repairCost" :required="true" class="selectTyp mb-2.5" :label="'Repair ($/' + area_unit + ')'" @blur="validateAndUpdateEcoParams('repairCost', 'Repair Cost', $event, 0, 10000)" />
              <number-input v-model="form.areaToRepairPct" :tool-tip-text="helping_tips.area_to_repair_default_settings" :min="0.1" :max="100" :rules="`required|min_value:0.1|max_value:100`" :title="`Allowable range of values is [0.1 , 100]`" step="any" :error="form.errors.areaToRepairPct" :required="true" class="selectTyp mb-2.5" label="Area to repair (%)" @blur="validateAndUpdateEcoParams('areaToRepairPct', 'Area to repair percentage', $event, 0.1, 100)" />
              <number-input v-model="form.repairIntervalYrs" :tool-tip-text="helping_tips.fixed_repair_intervals_default_settings" :min="1" :max="500" :rules="`required|min_value:1|max_value:500`" :title="`Allowable range of values is [1 , 500]`" step="1" :error="form.errors.repairIntervalYrs" :required="true" class="selectTyp mb-2.5" label="Fixed repair intervals (yrs)" @blur="validateAndUpdateEcoParams('repairIntervalYrs', 'Repair Interval', $event, 1, 500)" />
            </div>
          </div>
          <div class="w-full border-b flex flex-nowrap items-center bg-gray-50 py-1 px-2.5">
          ...
          <div class="flex w-full border-b lg:flex md:flex justify-between items-center bg-gray-50 py-1 px-2.5">
            <span style="font-weight: 500" class="btn-indigo-flat mr-1 mb-1 px-4 py-2 text-sm leading-4 text-gray-600 border reset-btn-mobile" @click="resetDefaults()">Reset to Life-365™ default values</span>
            <div class="flex">
              <loading-button id="save-button" :loading="form.processing" type="submit" class="btn-indigo-flat mr-2">OK</loading-button>
            </div>
          </div>
