    ...
    <div class="w-full max-w-md">
      <form class="mt-8 bg-white rounded-lg shadow-xl overflow-hidden" @submit.prevent="register">
        <div class="px-10 py-12 inst-login-block">
          ...
          <text-input minlength="8" v-model="form.password" :required="true" :error="form.errors.password" class="mt-2" label="Password" type="password" autofocus autocapitalize="off" />
          <text-input minlength="8" v-model="form.password_confirmation" :required="true" :error="form.errors.password_confirmation" class="mt-2" label="Confirm Password" type="password" autofocus autocapitalize="off" />
        </div>
