<template>
  <div>
    ...
    <div v-if="$page.props.flash.warning && show" class="full-width flex items-center justify-between bg-yellow-500 mb-5 rounded">
      <div class="flex items-center">
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="full-width flex items-center justify-between bg-red-400 mb-5 rounded">
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.props.errors).length === 1">Please fill valid data.</span>
          <span v-else>Please fill valid data.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
...
<script>
  export default {
    ...
    methods:{
      // close sesssion message function
      unShowSessionMessage: function () {
        var v = this;
        setTimeout(function () {
                v.show = false;
              }, 3000);
