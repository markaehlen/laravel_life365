<template>
  <div class="inline-editor" :class="classes">
    <div v-if="editingValue !== null">
      <input type="text" v-model="editingValue" :error-messages="textValidationErrors" @change="emitChange" @blur="emitBlur" @keypress="validateInput" @keyup.enter="updateValue" @keyup.esc="cancelEdit" :placeholder="placeholder" :required="required" ref="input" :autofocus="autofocus" />
...
export default {
  ...
  methods: {
    ...
    filterValue() {
      ...
      if (['currency', 'percentage'].indexOf(this.type) > -1) {
        this.editingValue = this.editingValue.toString().replace(/[^0-9.]/g, '')
      } else if (['number'].indexOf(this.type) > -1) {
        this.editingValue = this.editingValue.toString().replace(/^-?[^0-9.]+[^0-9.]/g, '')
      }
    },
    focus() {
      try {
    ...
    updateValue() {
      ...
      if (this.existingValues) {
        let trimmedValue = this.editingValue.replace(/\s+/g, ' ').trim()
        if (this.existingValues.indexOf(trimmedValue.toLowerCase()) !== -1) {
    ...
    validate() {
      ...
      } else if (this.min && this.editingValue && this.editingValue < this.min) {
        errors.push('Must be at least ' + this.min)
      } else if (this.max && this.editingValue && this.editingValue > this.max) {
        errors.push('Max ' + this.max + ' allowed')
      }
