...
class Payment
{
    ...
    function __construct($is_sendbox = null)
    {
        ...
        $stripe_secret_key = config("get.STRIPE_SECRET_KEY"); //"sk_test_51Hrw5CFjj0zdci6k9XG6mctHjCGSlmsTzBQy9FKUAMoxqCjF1MuWXD72qcZFcTr5skJuEPxLyjN3L1107I6PHLmm00sc7Bu8ei";
    ...
    public function initialRequirements($user, $account_id = null)
    {
        ...
        if (!empty($user->defaultAddress->state)) {
            $company['company']['address']['state'] = $user->defaultAddress->state;
        }
        ...
        if ($company['country'] == "US" || $company['country'] == "AU") {
            if (!empty($user->company_registration_number)) {
                $company['company']['registration_number'] = $user->company_registration_number;
            }
        }
    ...
    public function createAccountPersons($account_id, $person_id, $user, $type = null)
    {
        ...
        $existAddr = [];
        if (!empty($user->defaultAddress->address_line_1))
            $existAddr[] = $user->defaultAddress->address_line_1;
        if (!empty($user->defaultAddress->address_line_2))
            $existAddr[] = $user->defaultAddress->address_line_2;
        if (!empty($user->defaultAddress->city))
            $existAddr[] = $user->defaultAddress->city;
        if (!empty($user->defaultAddress->state))
            $existAddr[] = $user->defaultAddress->state;
        if (!empty($user->defaultAddress->postal_code))
            $existAddr[] = $user->defaultAddress->postal_code;
        if (!empty($user->defaultAddress->country->title))
            $existAddr[] = $user->defaultAddress->country->title;
        ...
        if ($user->location->iso_alpha2_code == "US not idea what it used") {
            ...
            if (!empty($user->defaultAddress->address_line_1)) {
                $person['address']['line1'] = $user->defaultAddress->address_line_1;
            } else {
                $person['address']['line1'] = implode(", ", $existAddr);
            }
            $person['address']['postal_code'] = $user->defaultAddress->postal_code ?? "";
            $person['address']['city'] = $user->defaultAddress->city ?? "";
        }

        if (!empty($user->defaultAddress->state)) {
            $person['address']['state'] = $user->defaultAddress->state;
        }
        ...
        if (!$type) {
            ...
            $person['relationship']['executive'] = true;
    ...
    public function createConnectedAccount($user, $account_id = null)
    {
        ...
        if ($itemInfoArr['country'] == "US" || $itemInfoArr['country'] == "GB" || $itemInfoArr['country'] == "BE" || $itemInfoArr['country'] == "AU") {
